  ...
  return (
    <div className="max-w-[760px] mx-auto px-6">
      ...
      <header className="text-center py-20 border-b border-rule">
        <p className="font-serif italic text-orange text-[1rem] mb-4" aria-hidden="true">
        ...
        {/* decorative dot-rule */}
        <div className="flex items-center justify-center gap-3 mb-5 mx-auto w-[120px]" aria-hidden="true">
      ...
      {/* Holidays */}
      <Chapter heading={dict.home.holidays_heading} id="holidays-heading" />
      <EntryGrid
        entries={holidays}
        locale={locale}
        lang={lang}
      ...
      {/* Lifecycle */}
      <Chapter heading={dict.home.lifecycle_heading} id="lifecycle-heading" />
      <EntryGrid
        entries={lifecycle}
        locale={locale}
        lang={lang}
...
function Chapter({ heading, id }: { heading: string; id: string }) {
  return (
    <div className="flex items-center gap-4 mt-12 mb-5">
      <h2 id={id} className="font-serif italic text-[1.25rem] text-ink-soft whitespace-nowrap">
        {heading}
      </h2>
  ...
  return (
    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-[10px]">
      {entries.map((entry) => {
        ...
        const customCount = entry.customs.length;
        const badgeText = hasCustums
          ? `${customCount} custom${customCount !== 1 ? "s" : ""}`
          : noCustomsLabel;
        return (
          <li key={entry.id}>
            <Link
              href={`/${lang}/${entry.slug}`}
              aria-label={`${entry.name[locale]}${hasCustums ? ` — ${badgeText}` : ""}`}
            ...
            >
              <span className="font-serif text-[0.97rem] text-ink" aria-hidden="true">
                {entry.name[locale]}
              </span>
              <span
                className={`text-[0.7rem] ${hasCustums ? "text-orange font-semibold" : "text-ink-light"}`}
                aria-hidden="true"
              >
                {badgeText}
              </span>
